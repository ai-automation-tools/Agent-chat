# AGENTS.md — agent_chat tester (kimi)
...
- `--agent-id`: **`kimi`**
- Kimi **auto-loads the project-scoped `.kimi-code/mcp.json`** from the launch directory (merged with the user-scoped `~/.kimi-code/mcp.json`). This tester's `agent_chat` registration lives in `.kimi-code/mcp.json` (this folder), so launching `kimi` from here picks it up — no config flag needed. The launcher and DB path there must match the values used by the other tester agents.
- The shared SQLite DB lives at `D:/AI_Agents/Projects/Mikes_AI_Lab/Repos/Live_Apps/Agent-Chat/db/chat.db`.

> **If `agent_chat` isn't listed** (`/mcp` shows the servers): confirm `.kimi-code/mcp.json` (this folder) has an `mcpServers.agent_chat` block and that you launched `kimi` from this folder. See `docs/CLI-MCP-Config/Per-CLI/kimi.md`. You must also have run `kimi login` once — Kimi auth is device-code, persisted to `~/.kimi-code/config.toml`; there is no API-key env var.

## How to participate in a conversation

1. **Fetch the kickoff once**: call `get_kickoff()` at the top of your session. It returns either the rendered prompt body the operator prepared (`status="ok"`) — follow the loop instructions it contains — or a generic fallback (`status="fallback"`) pointing at `prompts/Kickoff/kickoff.md`. If you get `status="no_conversation"`, ask the operator to seed one first.
