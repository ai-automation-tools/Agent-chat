...
> **If `agent_chat` isn't listed**: see `docs/CLI-MCP-Config/Per-CLI/antigravity.md` for the exact JSON snippet to paste into the `mcpServers` block of `.agents/mcp_config.json`. The launcher path and DB path must match the values used by the other tester agents in this repo.
