...
- **Per-machine config is ONE page.** `/settings` has three server-side tabs (`?tab=clis|notifications|delivery`) over the three things in `config/`. Tabs are **links, not JS** — only the selected tab's markup and script reach the page, so three forms written months apart can never collide on an element id and the HTML-string tests keep working. `/setup` and `/notifications` **302** to their tabs (`settings.LEGACY_PATHS`, pinned by a test) because they are linked from the README, the docs tree and bookmarks. A new setting is a tab entry plus a body builder; **a secret, a deployment posture or a launch argument is not a setting** and stays in the environment — see `docs/App/settings.md` for the line.
- **The Delivery tab owns sinks BY POSITION, the Notifications tab by ID.** Folder and command sinks predate any UI and carry no `id`, so `api/delivery_settings.py` edits the *first* of each type in place, preserves keys the form doesn't know, and reports later ones in `extra_sinks` rather than touching them. Tagging them on save would silently rewrite hand-written configs — the one thing a form over a gitignored, history-less file must not do. It never touches the webhook sink; two pages writing one sink is how that file loses work.
...
- **A seat is created by cloning seat 1's config — from one of THREE places.** `add_agent_seat._seat_one_config_text()` tries `CliShape.source_override` (an explicit global path — Codex's `~/.codex/config.toml`), then the seat-1 folder, then `CliShape.user_scope_entry` (a callable returning the tool's user-scope `agent_chat` entry, from which a minimal config is **synthesized**). The third exists because Claude Code's seat 1 is *expected* to have no project `.mcp.json` — one there makes it prompt for approval on every launch and stall a spawned agent — so cloning a file that must not exist made a second Claude Code seat impossible on every machine. A new CLI that registers globally must declare one of the two fallbacks; `tests/test_seats.py` pins the set against preflight's. Only the `agent_chat` entry is carried across, never the rest of `~/.claude.json`.
- **The form picks TOOLS; seat ids are derived.** `/orchestrate`'s Participants section is one row per **chair** — a dropdown of this machine's CLIs, that chair's persona, its custom-card button, a remove button — not a checkbox per seat id. The seat number comes from chair order (`renderSeatIds()` / `roomSeats()` in `web/render/orchestrate.py`): the second chair on a tool is `claude-code-2`, shown read-only in the row. So **the host may run on the same tool as a guest** — it just takes its own seat number — and a one-CLI operator can fill a whole room. `POST /api/orchestrate` creates any missing seat folder **before preflight**, through `web.api.setup._add_seat_module().add_seat()` — never a second copy of that logic. Consequences: nothing may re-introduce a control that picks a seat *id*, personas are a column of the chair row (there is no separate Personas section to keep in sync), and the per-tool ceiling is `seats.MAX_SEATS_PER_CLI`, enforced in the form before submit **and** by `seats.seat_id()` on the server.
- **Nothing *runs* the form's JS.** `tests/` asserts on rendered HTML strings, so a selector that hides the right markup passes every suite — that is exactly how the host seat picker shipped invisible (`updatePersonaRows()` caught the moderator's rows too). `tests/test_orchestrate_form.py` now pins the string invariants a browser would make redundant — every id / `[name=…]` / `.class` the script looks up resolves against the page, no bulk query runs at document scope, and a bulk-hidden class stays inside the container that owns it (`_BULK_HIDDEN_CLASSES`) — but **behaviour is still unexercised**. Say so when reporting work on `web/render/orchestrate.py`, and re-check the page in a browser rather than trusting a green run.
...
- **The panel is a package.** `src/panel/panel.js` is wiring + init; the work is nine ES modules in `src/panel/lib/`. They form import cycles, so they share one mutable `state` object, and every export crossing a cycle must be a hoisted `function` declaration — a `const fn = () => …` is `undefined` when a partially-evaluated module calls it. Composer insertion lives in `lib/compose.js` and nowhere else. **And every extension API call goes through `ext`** — exported by `lib/state.js` as `typeof browser !== 'undefined' ? browser : chrome`. Firefox exposes `chrome.*` only as a callback-based porting aid, so a bare `chrome.` is `undefined` on the first `await` and the panel never paints (it shipped that way from 2026-08-01 to 2026-09-08). `background.js` keeps its own copy — classic script on Gecko, must stay import-free.
- **Nothing under `extension/` has a test that runs in a browser.** `tests/test_battleground.py` reaches the bridge and the MCP loop only, and the Firefox-namespace guard is a grep. Say so when reporting extension work. What *has* been exercised for real is the **Chrome** loop — five `posted` drafts on live Reddit threads against today's `extension/src/` (see the Roadmap row); **Firefox has never completed a run**, so don't describe the Gecko build as verified.
...
- **Notifications are ONE sink of this, not a second system.** the Settings → Notifications tab + `web/api/notifications.py` render `config/delivery.json` as a form and own exactly the sink tagged `delivery.NOTIFY_SINK_ID`. **Saving merges** — a hand-written folder/command sink survives, which is load-bearing for a gitignored file with no history. Services are a *rendering* (`SERVICES`), never a sink type: `body: "text"` + a templated `template`/`headers` covers ntfy, `text_key` covers Slack/Discord/Gotify, and a per-service adapter for each would be four modules differing by one string. A template's unknown `{placeholder}` renders **empty, never raises** — `quiet_seconds` exists only on `stalled`, and a config that dies on the day it matters is worse than none. `send_test()` deliberately **raises** where `deliver()` swallows; a human is waiting for the answer. `optin_offered()` **excludes** the notify sink by id — it's scoped `"all"`, and counting it makes `/orchestrate`'s *save a copy* box read as forced.
- **`started` is the fourth event and the fourth call site** — `seeding.seed_conversation()`, after the connection closes. It is **not** in the default `events` list on purpose: at seed time the bundle has no messages, so a folder sink inheriting it writes an empty transcript. Pinned by `tests/test_notifications.py`, which seeds for real rather than grepping for the call.
- **Four call sites for the three END paths, and a test pins all three**: `send_message()`, `web.db.stop_conversation()`, `inspect_conversations.cmd_stop()`. Adding another way to end a conversation means adding another `deliver()` call. Do **not** hook `get_my_turn()` — it re-runs `maybe_complete()` on every poll and would re-deliver on a loop — and always call it **outside** the write transaction.
...
- **A run may hold several results, and the LAST one wins.** Leads draft-then-revise — every collaboration so far has (#51 six, #54 two), each revision better than the last. `export.final_result()` / `superseded_result_ids()` define last-wins **once**, for the reader, the export and the delivery sink. The export publishes which is current as an **additive `Result` row in `topic.md`**, never as a transcript-heading suffix: the contract says a heading *ends* with the `` `signal=<value>` `` span, so a suffix could break an end-anchored parser. `tests/test_deliverable_flow.py` pins the headings clean.
- **A non-lead cannot `signal='done'` before a result exists** in a `produces_deliverable` type — `blocks_premature_done()` refuses it *before* the insert. Otherwise a collaborator can close a run that produced nothing, and the row still reads `complete`. The lead may always end early, `blocked` is never restricted, and transcript types (debate/podcast) are untouched.
- **A type may declare EXTRA roles beyond the lead and the members** — `ConvType.extra_roles`, a tuple of `ExtraRole(role, label, plural, max_count, hint)`. `roles` is a variable-length tuple, not the old 2-tuple; that limit was what blocked red-team, a differentiated expert panel and the designated skeptic. An extra role **re-brands a member seat, never adds one**: `collaborate`'s optional `skeptic` is one of the collaborators the operator already picked, so nothing about the participant bounds changes. Consequences that are easy to get wrong: it is **never assigned implicitly** (`default_roles()` still only ever produces the lead and the member role, so a caller that says nothing gets the room it always got) · `min_members` counts the **plain** member role, so a facilitator plus a skeptic is refused · the lead cannot also hold one, and `POST /api/orchestrate` says so rather than letting a dict update pick a winner · every extra role needs its own `_ROLE_BRIEFS` entry, which `tests/test_conv_types.py` pins via `ALL_ROLES`. The `/orchestrate` *Special seats* dropdowns and its `roles: {cli: role}` payload generate from the registry — adding one costs no render edit and **no PowerShell edit**.
...
| `test_availability.py` | CLI detect-vs-declare, `plan_seats` round-robin, the Settings CLI-tools tab + its legacy `/setup` redirect, `/orchestrate` filtering, demo strip, two-group rail, `CLI_BINARIES` ↔ `spawn-agents.ps1` parity, **lifecycle scripts match processes by command line not `ExecutablePath`** |
| `test_media_prompts.py` | Image/audio prompt builders + the `/prompts/{kind}.md` route |
| `test_orchestrate_form.py` | `/orchestrate`'s inline JS ↔ its markup, by string invariants — lookups resolve, conditional controls are null-guarded, no document-scope bulk query, bulk-hidden classes stay in their container. **Does not run the JS.** |
...
| `test_notifications.py` | The notify sink over a **real loopback HTTP server**: ntfy's text body + templated headers, the Discord/Slack one-key difference, seeding firing `started`, a dead endpoint not failing a seed, config merge preserving hand-written sinks |
...
| `test_db_sync_watermarks.py` | Sidecar watermarks: push cursor on the **local** clock, pull cursor on the server's; a local edit behind `server_time` still pushes; the pulled-row echo terminates; `--force-push`; **`config/sync-exclude.json` keeps a conversation off the public mirror without deleting it locally** |
...
| [`docs/App/settings.md`](docs/App/settings.md) · [`notifications.md`](docs/App/notifications.md) | The `/settings` tab shell and what isn't a setting; the four events and the two webhook body modes. |
