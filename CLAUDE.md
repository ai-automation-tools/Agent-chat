...
- **Nothing tests the form's JS.** `tests/` asserts on rendered HTML strings, so a selector that hides the right markup passes every suite — that is exactly how the host seat picker shipped invisible (`updatePersonaRows()` caught the moderator's rows too). Say so when reporting work on `web/render/orchestrate.py`, and re-check the page in a browser rather than trusting a green run.
