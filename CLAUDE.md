...
- [`.claude/skills/`](.claude/skills/) — read by **Claude Code working on this repo**. Project-specific: `agent-chat-schema`, `agent-chat-export-contract`, `agent-chat-web-ui`. Plus `.claude/agents/agent-chat-docs-sync` (audits a diff for doc drift) and `.claude/commands/` (`/smoke-test`, `/deploy-fly`, `/close-roadmap-item`).
