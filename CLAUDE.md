...
- **An arena can be cast with a persona that isn't in the registry.** The panel's `✎ custom instructions…` option sends `persona_instructions` instead of `persona`, and it lands in the same snapshot columns with **`persona_slug` left NULL** — that's why it needs no schema and no registry write. Three things follow, each of which a plausible "cleanup" would break: `get_arena` gates the persona on **`persona_body`, never `persona_slug`** (gating on the slug hands a custom-cast agent `persona: null`); re-casting onto a custom card must pass `bg_update_arena`'s `clear_persona_slug`, since "skip on None" would otherwise leave the old card's slug beside the new name; and passing both `persona` and `persona_instructions` is a **400, not a precedence rule**. Never write a one-off card into the `personas` table — casting an arena is not authoring a persona.
- **The panel's tab→arena link is keyed on tab id alone.** `linkArena()` stores a `url` beside the arena id but nothing reads it back, so navigating a tab to a new page keeps the old arena attached. **↺ Start over** (`#unlink`) is the way out, and it must clear `state.capture` as well as `state.arena` — `render()` shows the Cast card whenever there's a capture and no arena, so dropping only the arena re-offers the *previous* page's posts with **Open arena** live. `clearCapture()` in `panel.js` is shared with the tab-switch path; keep both callers on it. Start over is deliberately **not** destructive — the arena and its drafts survive on the server, and **Close arena** is the separate control that stops the agent drafting.
