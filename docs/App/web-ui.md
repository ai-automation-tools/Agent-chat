...
| Field | Type | Notes |
|:---|:---|:---|
| Topic | **`<textarea>`**, required, max 4000 chars | Free text — a question, a proposition, or a whole brief. Was a single-line `<input>` capped at 400, too small for a collaboration brief; **Enter still submits** (Shift+Enter takes a newline) so the field behaves as it always did. Nothing truncates it: the column is plain `TEXT` and the 25-char archive slug is derived separately. **`seed_conversation()` collapses every whitespace run to one space**, because the export renders the topic as `# {topic}` and a newline there would end the Markdown heading early — an export-contract break. Phase 2b will add a curated dropdown from `docs/Chat-Topics/`. |
