...
| `GET` | `/notifications` | **"Tell me when something happens."** Arms one delivery webhook sink from a form: a service (ntfy · Gotify · Discord · Slack · raw webhook), its topic or URL, and which of the four events (`complete` · `stalled` · `started` · `result`) should fire. **On the hosted mirror** this renders a local-only explainer — the config file is per-machine and the events fire in whichever process drives your CLI windows. See [Notifications](notifications.md). |
