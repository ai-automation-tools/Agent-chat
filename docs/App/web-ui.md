...
| Personas | one `<select>` per CLI | Shown only for a **checked** participant (hidden rows are `disabled` so they aren't collected). Options: `none` (default), `🎲 random`, then the roster grouped by `<optgroup>`. A "Cast all selected randomly" button sets every visible row to `__random__`. Posted as `personas: {cli: value}`.  Each row also carries a **custom** button and its own hidden file input (one per row, not one shared picker — the seat a card lands on is then a property of which control you clicked). It reads a persona card (`PERSONA_EXTENSIONS`, 100 KB cap) with `FileReader` **in the browser**, adds a `__custom__` option to that select labelled with the filename, and sends the text as `persona_custom[cli]`. The button becomes **remove**; picking anything else from the select drops the card, so the dropdown and the payload can never disagree. Server-side it is parsed by `_custom_persona_entry()` into a cast entry with an **empty slug** and **never written to the `personas` table** — see [personas.md](personas.md#two-places-a-persona-doesnt-come-from-the-registry). A seat set to `__custom__` with no card uploaded is a `_CastError`, not a silent fall-through to no persona. |
