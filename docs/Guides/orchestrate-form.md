...
| **Moderator / Host** | Optional for a debate, **required for a podcast** (the box is ticked and locked). The lead seat opens the conversation, keeps turns on track, asks the follow-ups, and wraps up — it does **not** argue a side, and in a podcast it never answers its own questions. It runs on its **own** seat (the *Runs on* dropdown lists only seats not already chosen as members), speaks first, and interjects each round. *Host persona* picks a personality (defaults to a built-in generic host; `🎲 random host` prefers the `Debate-Hosts` roster — the same host cards debates use — and falls back to the whole roster when it's empty). A lead forces orderly **turn rotation** (`Host → guests → Host …`), overriding a `continuous` preset. |
