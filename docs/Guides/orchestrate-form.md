...
box, a row per chair with a dropdown of the CLI tools you have, and preset
dropdowns.
...
- You've told the app which CLIs you have, on [**Settings → CLI tools**](../App/cli-setup.md).
...
| Field | Notes |
|:--|:--|
| **Topic** | Free text — the prompt for the room. Required. A resizable box holding up to **4000 characters**, so a full brief fits, not just a headline. **Enter submits**; Shift+Enter gives you a newline while drafting (line breaks are collapsed to spaces when it's saved). For a long standing brief, the *optional system message* at the bottom is often the better home — it arrives as the conversation's first message rather than as its title. |
...
| **Participants / Guests** | **One row per chair.** Pick the tool each chair runs on from its dropdown, add rows with **+ Add a seat**, drop one with **×**. **Only the CLIs you have are listed** — whatever you ticked on [Settings → CLI tools](../App/cli-setup.md), or what was detected if you never answered. A tool whose config didn't pass the page-load check is still offered, with the reason on it (`codex — config_missing`); the real check runs on submit and a failure there aborts the whole launch. **The same tool can fill more than one chair** — that's how a single CLI runs a whole room. You don't name seats: the second chair on a tool becomes `claude-code-2` and the row tells you so (*runs as claude-code-2*), and its config is created for you when you launch. A banner appears above the list when there's nothing runnable or you haven't confirmed your setup; a settled setup gets no banner. |
| **Sub-type** | A radio grid of the presets the chosen format offers — one for debate, one for podcast, nine for collaboration (`collaborate`, `brainstorm`, `plan`, `decide`, `solve`, `code-review`, `audit`, `design`, `validate`), plus `None`. Every card carries a **`?` in its bottom-right corner** — **hover it** for the long form: what you hand in, what comes back, and which neighbouring sub-type to pick instead. It floats over the card rather than expanding it, so nothing on the page moves, and reading one never selects it. Sets the tone and, for a collaboration, the shape of the closing `signal='result'` deliverable. Picking one auto-fills the max-turns box with that preset's default and applies its mode. The section hides itself for a format with only one preset. |
...
| **Personas (optional)** | A dropdown in every chair row. Pick a specific personality (grouped by persona group), `🎲 random` (draws an unused persona from the roster), or `none`. **🎲 Cast every chair randomly** sets every row to random in one click. The chosen persona is woven into that agent's opening prompt so it debates in character. |
| **Special seats** | Only appears for a format that has one — today just a collaboration's optional **Skeptic**. It is **not an extra chair**: the dropdown lists the seats you already checked, minus whoever will facilitate, and the one you pick gets a different brief (lead with the objection, name the specific defect, bring the alternative). Leave it on *(none)* and the room behaves exactly as before. See [the collaboration guide](collaborate.md#the-designated-skeptic). |
| **Moderator / Host** | Optional for a debate, **required for a podcast** (the box is ticked and locked). The lead seat opens the conversation, keeps turns on track, asks the follow-ups, and wraps up — it does **not** argue a side, and in a podcast it never answers its own questions. It runs on its **own** seat — but *Runs on* is a list of **tools**, like every chair, so the host may share a tool with the guests: it simply takes its own seat number (host on `claude-code`, the first guest on that tool becomes `claude-code-2`). It speaks first and interjects each round. *Host persona* picks a personality (defaults to a built-in generic host; `🎲 random host` prefers the `Debate-Hosts` roster — the same host cards debates use — and falls back to the whole roster when it's empty). A lead forces orderly **turn rotation** (`Host → guests → Host …`), overriding a `continuous` preset. |
| **Launch** | **Spawn one CLI window per agent** (auto-open a terminal per participant — local Windows only) and **Skip tool-approval prompts** (hands-off `--yolo` / `--dangerously-skip-permissions`), both on by default = fully hands-off. Turn spawn off to seed only and launch manually. A third toggle, **Deliver a copy when this finishes**, appears only once you've configured [delivery](../App/delivery.md) — it writes the export bundle to `deliveries/` (or fires a webhook or a command) when the run ends, so you don't have to come back and click Export. |
...
- [`docs/CLI-MCP-Config/`](../CLI-MCP-Config/README.md) — register `agent_chat` per CLI (what preflight checks).
